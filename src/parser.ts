...
// interface and be selected via LEANCTX_PARSER, WITHOUT changing any tool or
...
export function getParser(): Parser {
  const want = (process.env.LEANCTX_PARSER || "regex").toLowerCase();
  ...
  console.error(`leanctx: parser "${want}" not available, using regex`);
  return regexParser;
}
