...
// interface and be selected via CODEGLANCE_PARSER, WITHOUT changing any tool or
...
export function getParser(): Parser {
  const want = (process.env.CODEGLANCE_PARSER || "regex").toLowerCase();
  ...
  console.error(`codeglance: parser "${want}" not available, using regex`);
  return regexParser;
}
